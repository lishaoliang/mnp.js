 ...
 * 缓存: [2-4)帧
 * 延时: [66-132]毫秒/30fps
 * 抗抖动: 99毫秒/30fps, [3*(1000/30)]
*/

#define FF_RATE_CTRL_LOWEST_FRAME_MAX        45


static em_frame_yuv_wav_t* ff_rate_ctrl_lowest_pop_frame(ff_rate_ctrl_t* p_rate_ctrl, int64_t now_ticks)
{
...
static em_frame_yuv_wav_t* ff_rate_ctrl_lowest_get_frame(ff_rate_ctrl_t* p_rate_ctrl, int64_t now_ticks)
{
    ...
    {
        // 如果YUV的头帧时间戳为0, 则直接取走
        return ff_rate_ctrl_lowest_pop_frame(p_rate_ctrl, now_ticks);
    }
    ...
    if (0 == p_rate_ctrl->base_ticks || 0 == p_rate_ctrl->base_pts)
    {
        if (2 <= all_size)
        {
            FF_RATE_CTRL_LOG("C ff_rate_ctrl_lowest_get_frame base");
    ...
    else
    {
            ...
            FF_RATE_CTRL_TIME_MS * 1333 <= ABS_SUB(p_yuv_head->pts, p_rate_ctrl->last_pts))
        {
            ...
            {
                FF_RATE_CTRL_LOG("C ff_rate_ctrl_lowest_get_frame pts error");
        ...
        else
        {
            ...
            if (FF_RATE_CTRL_STATUS_UNIFORM == p_rate_ctrl->status)
            {
                if (3 < all_size)
                {
                    FF_RATE_CTRL_LOG("C ff_rate_ctrl_lowest_get_frame FF_RATE_CTRL_STATUS_UNIFORM to FF_RATE_CTRL_STATUS_FAST,all_size:[%d]", all_size);
                    p_rate_ctrl->status = FF_RATE_CTRL_STATUS_FAST;
                }

                if (0 <= all_size && all_size < 2)
                {
                    FF_RATE_CTRL_LOG("C ff_rate_ctrl_lowest_get_frame FF_RATE_CTRL_STATUS_UNIFORM to FF_RATE_CTRL_STATUS_SLOW,all_size:[%d]", all_size);
            ...
            if (FF_RATE_CTRL_STATUS_FAST == p_rate_ctrl->status)
            {
                ...
                if (dt_pts <= dt_ticks + FF_RATE_CTRL_TIME_MS * 6)
                {
                    ...
                    if (uniform)
                    {
                        FF_RATE_CTRL_LOG("C ff_rate_ctrl_lowest_get_frame FF_RATE_CTRL_STATUS_FAST to FF_RATE_CTRL_STATUS_UNIFORM,all_size:[%d]", all_size);
                        p_rate_ctrl->uniform_num = 0;
                        p_rate_ctrl->status = FF_RATE_CTRL_STATUS_UNIFORM;
                    }
            ...
            else if (FF_RATE_CTRL_STATUS_SLOW == p_rate_ctrl->status)
            {
                ...
                if (dt_pts <= dt_ticks + FF_RATE_CTRL_TIME_MS * 6)
                {
                    ...
                    if (uniform)
                    {
                        FF_RATE_CTRL_LOG("C ff_rate_ctrl_lowest_get_frame FF_RATE_CTRL_STATUS_SLOW to FF_RATE_CTRL_STATUS_UNIFORM,all_size:[%d]", all_size);
                        p_rate_ctrl->uniform_num = 0;
                        p_rate_ctrl->status = FF_RATE_CTRL_STATUS_UNIFORM;
                    }
...
static int ff_rate_ctrl_lowest_drop_data(ff_rate_ctrl_t* p_rate_ctrl, bool hidden)
{
    assert(NULL != p_rate_ctrl);
    ff_dec_single_t* p_ff_dec = p_rate_ctrl->p_ff_dec;
    assert(NULL != p_ff_dec);
    ...
    else
    {
        ...
        // 此时也需要丢帧
        if (FF_RATE_CTRL_LOWEST_FRAME_MAX < klb_list_size(p_ff_dec->p_video_list))
        {
            drop_num = ff_rate_ctrl_drop_to_last_key(p_ff_dec);
        }
    }
    ...
    if (0 < drop_num)
    {
        ...
        FF_RATE_CTRL_LOG("C ff_rate_ctrl_lowest_drop_data:[%d]", drop_num);
    }

    return drop_num;
}

static void ff_rate_ctrl_lowest_reset(ff_rate_ctrl_t* p_rate_ctrl)
{
    //LOG("C ff_rate_ctrl_lowest_reset");
...
{
    p_rate_ctrl->get_frame = ff_rate_ctrl_lowest_get_frame;
    p_rate_ctrl->drop_data = ff_rate_ctrl_lowest_drop_data;
    p_rate_ctrl->reset = ff_rate_ctrl_lowest_reset;
}
